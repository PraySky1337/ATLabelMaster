...
MainWindow::MainWindow(QWidget* parent)
    ...
    if (ui_->list_view) {
        ...
        connect(ui_->list_view->selectionModel(), &QItemSelectionModel::currentChanged,
    ...
    connect(this, &MainWindow::sigClassSelected, this, [this](const QString& name) {
        if (ui_->label) ui_->label->setCurrentClass(name);
    });

    // 智能标注
    connect(this, &MainWindow::sigSmartAnnotateRequested,
            ui_->label, &ImageCanvas::requestDetect);
...
void MainWindow::appendLog(const QString& line) {
    ...
    if (auto* te = ui_->log_text) te->append(s);
}
...
void MainWindow::setFileModel(QAbstractItemModel* model) {
    ...
    connect(sel, &QItemSelectionModel::currentChanged,
            this, [this](const QModelIndex& cur, const QModelIndex&) {
                if (cur.isValid()) emit sigFileActivated(cur);
            });

    // 5) 视图样式
    tv->header()->setStretchLastSection(false);
    tv->header()->setSectionResizeMode(0, QHeaderView::Stretch);
    for (int c = 1; c < model->columnCount(); ++c) tv->setColumnHidden(c, true);
...
void MainWindow::setBusy(bool on) {
    if (on) QApplication::setOverrideCursor(Qt::WaitCursor);
    else    QApplication::restoreOverrideCursor();
}

void MainWindow::setUiEnabled(bool on) {
    if (auto* w = centralWidget()) w->setEnabled(on);
}
...
void MainWindow::setCurrentClass(const QString& name) {
    if (!clsModel_ || !ui_->list_view) return;
    const auto list = clsModel_->stringList();
    const int row = list.indexOf(name);
    if (row >= 0) ui_->list_view->setCurrentIndex(clsModel_->index(row));
}
...
void MainWindow::setLogTimestampEnabled(bool on) {
    logTimestamp_ = on;
}

bool MainWindow::textInputHasFocus() const {
    QWidget* w = QApplication::focusWidget();
    return w && (w->inherits("QLineEdit") || w->inherits("QTextEdit") || w->inherits("QPlainTextEdit"));
}

void MainWindow::keyPressEvent(QKeyEvent* e) {
    if (textInputHasFocus()) { QMainWindow::keyPressEvent(e); return; }
    ...
    switch (e->key()) {
    case Qt::Key_Q: emit sigPrevRequested();         e->accept(); return;
    case Qt::Key_E: emit sigNextRequested();         e->accept(); return;
    case Qt::Key_O: emit sigOpenFolderRequested();   e->accept(); return;
    case Qt::Key_S: emit sigSaveRequested(); ui_->label->publishAnnotations(); e->accept(); return;
    case Qt::Key_H: emit sigHistEqRequested();       e->accept(); return;
    case Qt::Key_Delete: emit sigDeleteRequested();  e->accept(); return;
    case Qt::Key_Space:  emit sigSmartAnnotateRequested(); e->accept(); return;
    case Qt::Key_F1:     emit sigSettingsRequested(); e->accept(); return;
    case Qt::Key_A: {
        ...
        if (cls.isEmpty()) cls = QStringLiteral("unknown");
        setStatus(tr("开始标注：%1（拖一个矩形，然后拖拽角点精调，右键/ESC取消）").arg(cls), 2000);
        e->accept(); return;
    }
    default:
        emit sigKeyCommand(QKeySequence(e->key()).toString());
        break;
    }
...
void MainWindow::dragEnterEvent(QDragEnterEvent* e) {
    if (!dragDropEnabled_) { e->ignore(); return; }
...
void MainWindow::dropEvent(QDropEvent* e) {
    ...
    if (!paths.isEmpty()) emit sigDroppedPaths(paths);
...
static QAction* ensureAction(QAction* act, const QKeySequence& ks, const QString& tip) {
    ...
    if (!ks.isEmpty()) act->setShortcut(ks);
